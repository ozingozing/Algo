...
BTreeNode* splitNode(int pos, BTreeNode* node, BTreeNode* parent) { // 현재 노드, 부모 노드, 현재노드에서 넣은 값의 위치를 알고있는 pos를 인자로 받기
    int median; // 분리를 위해 중앙값 알아야 함.
    median = node->cnt_key / 2; // 중앙값은 짝수이든 홀수이든 '키개수/2' 
    BTreeNode* right_node = new BTreeNode; // 분리 한 값을 새로 넣어줄 오른쪽 노드 만듬.(추후 자식이 됨)
...
BTreeNode* insertNode(int parent_pos, int val, BTreeNode* node, BTreeNode* parent) { // 삽입할 값,  
...
// 왼쪽에서 빌리는 함수
void borrowFromLeft(BTreeNode* par_node, int cur_node_pos) { // 부모 노드와 현재 노드 pos위치를 인자로 받음. 이미 현재 노드의 키는 지워졌음. cnt_key는 최소상태 혹은 미만일거임.
...
// 오른쪽에서 빌리는 함수
void borrowFromRight(BTreeNode* par_node, int cur_node_pos) { // 부모 노드와 현재 노드 pos위치를 인자로 받음. 이미 현재 노드의 키는 지워졌음. cnt_key는 최소상태 혹은 미만일거임.
...
void balanceNode(BTreeNode* node, int child_pos) { // 현재 노드와 자식노드에서의 위치를 인자로 받는 함수(빌리기,병합을 진행)
...
// predecessor 찾는 함수 // 자기보다 작은 값 중 가장 큰 값
...
// suceessor 찾는 함수 //자기보다 큰 값 중 가장 작은 값
...
// 지우는 함수
void Delete(BTreeNode* node, int val) { // 현재 노드랑 지우려는 값을 인자로 받음.
...
// 트리 프린트 하는 함수
void printTree(BTreeNode* node, int level) {			 // B트리 그리기
    if (!node) { 											 // empty tree!
        printf("Empty tree!!\n");
        return;
    }
